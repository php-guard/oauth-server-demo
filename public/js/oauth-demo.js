...
$(function () {
    //TODO recuperer le fragment pour preremplir les input des grant types
    let $grantTypes = $('#grant-types');
    let $grantTypeSelected = $('#grant-type-selected');
    let $select = $('#access-token-request-grant-type');

    $select.change(function () {
        $grantTypeSelected.slideUp().children().appendTo($grantTypes);
        $('#grant-type-' + $(this).val().replace('_', '-')).appendTo($grantTypeSelected);
        $grantTypeSelected.slideDown();
    });
});
